...
const _scenarioPairs = <_ScenarioPair>[
  _ScenarioPair(
    ...
    v2Scenario: 'dynamic_card_50k_sliver_v3',
  ),
  _ScenarioPair(
    label: 'dynamic_chip_10k',
    v1Scenario: 'dynamic_chip_10k',
    v2Scenario: 'dynamic_chip_10k_sliver_v3',
  ),
];
...
Future<void> main(List<String> args) async {
  ...
  stdout.writeln('Running A/B benchmark for sliverV3 spike');
...
_Config _parseArgs(List<String> args) {
  ...
  var outlierIqrK = 1.5;
  var reportPath = 'benchmarks/sliver_v3_ab_report.md';
  var jsonReportPath = 'benchmarks/sliver_v3_ab_report.json';
...
void _printUsage() {
  stdout.writeln('Usage: dart run benchmarks/run_sliver_v3_ab.dart [options]');
  stdout.writeln();
  ...
  stdout.writeln(
    '  --report-path <p>        Markdown report output path (default: benchmarks/sliver_v3_ab_report.md).',
  );
  stdout.writeln(
    '  --json-report-path <p>   JSON report output path (default: benchmarks/sliver_v3_ab_report.json).',
  );
...
String _writeReport({
  ...
  final report = StringBuffer()
    ..writeln('# A/B Benchmark: offstageV1 vs sliverV3 (Spike)')
    ..writeln()
  ...
  for (final pair in pairs) {
    ...
    report
      ...
      ..writeln()
      ..writeln('### sliverV3 (`${pair.pair.v2Scenario}`)')
      ..writeln()
      ..writeln(
        '| run_id | p95_build | p95_raster | jank% | peak_mb | tti_ms |',
      )
      ...
      ..writeln()
      ..writeln('| Métrica | offstageV1 | sliverV3 | Mejora V3 vs V1 |')
      ..writeln('|---|---:|---:|---:|')
      ..writeln(
      ...
      ..writeln(
        'Muestras usadas sliverV3 (build/raster/jank/memoria/tti): '
  ...
  report
    ..writeln('## Resumen por escenario (mejora V3 vs V1)')
    ..writeln()
    ...
    ..writeln()
    ..writeln(
      '- Valor positivo en "Mejora" significa que `sliverV3` mejora (menor valor) respecto a `offstageV1` para esa métrica.',
    )
    ..writeln(
      '- Valor negativo en "Mejora" significa regresión de `sliverV3` respecto a `offstageV1`.',
    )
    ..writeln();
