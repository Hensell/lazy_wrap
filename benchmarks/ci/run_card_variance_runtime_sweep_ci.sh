...
OUT_DIR="${CI_OUT_DIR:-benchmarks/results/ci/$RUN_ID/card_variance_runtime_sweep}"
...
REPORT_MD="$OUT_DIR/card_variance_runtime_sweep_report.md"
CI_SUMMARY_MD="$OUT_DIR/card_variance_runtime_sweep_ci_summary.md"

mkdir -p "$SWEEP_OUTPUT_DIR"

echo "Card variance runtime sweep CI run"
...
echo "RUNTIME_PRESETS=$RUNTIME_PRESETS"
...
# Card Variance Runtime Sweep (Skipped in CI)
...
__REPORT__
  ...
  if [[ "$BENCH_STABLE_SCROLL_PROFILE" == "true" ]]; then
    SWEEP_ARGS+=(--stable-scroll-profile)
  fi

  if [[ -n "$RUNTIME_PRESETS" ]]; then
    IFS=',' read -r -a PRESET_LIST <<< "$RUNTIME_PRESETS"
  ...
  dart run benchmarks/run_card_variance_runtime_sweep.dart "${SWEEP_ARGS[@]}"
fi
...
# Card Variance Runtime Sweep CI Summary
...
echo "Card variance runtime sweep CI checks completed."
echo "Artifacts:"
